...
let intersectionObserver = null; // Pour la gestion des observateurs
...
function getCSRFToken() {
    ...
    // Si pas trouvé dans les cookies, chercher dans un input hidden
    if (!cookieValue) {
    ...
    console.log('CSRF Token trouvé:', cookieValue ? 'Oui' : 'Non');
    return cookieValue;
}
...
// =====================================================================
// UTILITAIRES POUR LA GESTION DES ÉVÉNEMENTS - REFACTORISÉS
// =====================================================================
/**
 * Ajoute les mêmes événements pour click et touch
 * @param {Element} element - L'élément à modifier
 * @param {string} eventType - 'click' ou 'touchstart'
...
/**
 * Ajoute un événement 'Enter' pour les champs de saisie
 * @param {Element} input - L'élément input
 * @param {Function} handler - La fonction à exécuter
 */
...
// Nettoyage des ressources avant le déchargement
...
function setupEventListeners() {
    ...
    // Mobile menu
    if (mobileMenuBtn) {
        addEventListeners(mobileMenuBtn, toggleMobileMenu);
    }

    // Navigation links
    navLinks.forEach(link => {
        addEventListeners(link, handleNavLinkClick);
    });
    ...
    // Back to top
    if (backToTopBtn) {
        addEventListeners(backToTopBtn, scrollToTop);
    }

    // Donation modal
    if (modalClose) {
        addEventListeners(modalClose, closeDonationModal);
    }

    // Gallery modal
    if (galleryModalClose) {
        addEventListeners(galleryModalClose, closeGalleryModal);
    }

    // Theme toggle
    if (themeToggle) {
        addEventListeners(themeToggle, toggleTheme);
    }

    // Program tabs
    const programTabs = document.querySelectorAll('.program-tab');
    programTabs.forEach(tab => {
        addEventListeners(tab, handleProgramTabClick);
    });
    ...
    if (newsletterForm) {
        ...
        if (newsletterBtn) {
            addEventListeners(newsletterBtn, (e) => handleNewsletterSubmit(e, newsletterInput, newsletterBtn));
        }
...
function handleNewsletterSubmit(e, input, btn) {
    ...
    if (email && isValidEmail(email)) {
        ...
        sendNewsletterData(email, btn, originalText, input);
    } else {
        showToast('Veuillez entrer une adresse email valide.', 'error');
    }
}

async function sendNewsletterData(email, btn, originalText, input) {
    try {
        ...
        console.error('Erreur d\'envoi newsletter:', error);
...
function setupCarouselEvents() {
    ...
    addEventListeners(carouselPrev, () => {
        if (currentSlideIndex > 0) {
            currentSlideIndex--;
            scrollToSlide(currentSlideIndex);
        }
    ...
    addEventListeners(carouselNext, () => {
        if (currentSlideIndex < carouselImages.length - 1) {
            currentSlideIndex++;
            scrollToSlide(currentSlideIndex);
        }
...
function optimizeTeamImages() {
    ...
    teamCards.forEach((card, index) => {
        ...
        // 2. Réinitialiser les styles du conteneur
        ...
        if (img) {
            ...
            // 4. Réinitialiser l'image
            img.className = 'team-img';

            // FORCE LE CARRÉ ARRONDI
            if (isMobile) {
                // Sur mobile : carré de 220px avec arrondi de 15px
                ...
                // Sur desktop : carré de 320px avec arrondi de 12px
...
function openGalleryModal(galleryItem) {
    if (!galleryModal || !galleryModalImg || !galleryModalCaption || !galleryModal) return;
...
// =====================================================================
/**
 * Corrections apportées et améliorations :
 * 
 * 1. REFACTORISATION DES ÉVÉNEMENTS TACTILES
 *    - Créé les utilitaires addEventListeners() et addEnterKeyListener()
 *    - Éliminé le code redondant pour les événements click/touch
 *    - Meilleure gestion des erreurs d'accès aux propriétés tactiles
 * 
 * 2. GESTION DES RESSOURCES
 *    - Ajout de cleanupResources() appelée au déchargement
 *    - Nettoyage des intervalles (aboutCarouselInterval)
 *    - Déconnexion des observateurs IntersectionObserver
 *    - Fermeture des modales ouvertes
 * 
 * 3. SÉCURITÉ DU CSRF TOKEN
 *    - Ajout de credentials: 'same-origin' aux requêtes fetch
 *    - Vérifications supplémentaires du token
 *    - Meilleure gestion des erreurs 403 et 404
 * 
 * 4. GESTION DES TOUCHES / SWIPE
 *    - Vérification null-safe pour e.touches et e.changedTouches
 *    - Utilisation de l'optional chaining (?.)
 *    - Prévention des erreurs undefined
 * 
 * 5. OBSERVATEURS INTERSECTION
 *    - Stockage de intersectionObserver en variable globale
 ...
 *    - Indication des étapes critiques
...
console.log('Application JavaScript initialisée avec succès');